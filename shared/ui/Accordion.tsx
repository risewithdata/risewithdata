      ...
      {items.map((item, index) => {
        ...
        return (
          <div key={item.question} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            ...
            >
              {item.question}
              <span className={`ml-4 inline-flex h-9 w-9 items-center justify-center rounded-full border ${isActive ? 'border-brand-600 bg-brand-600 text-white' : 'border-slate-300 text-slate-500'}`}>
                {isActive ? '−' : '+'}
              </span>
            </button>
