...
export async function GET() {
  try {
    ...
    const users = await prisma.$queryRawUnsafe<Record<string, unknown>[]>(
      `SELECT id, email, "firstName", "lastName", role, active, "createdAt"
  ...
  try {
    ...
    if (body.firstName !== undefined || body.lastName !== undefined) {
      ...
      await prisma.$queryRawUnsafe(
        `UPDATE "User" SET "firstName" = $1, "lastName" = $2, "updatedAt" = NOW() WHERE email = $3`,
        first,
        last,
        session.email
      );
    ...
    if (body.currentPassword && body.newPassword) {
      ...
      const users = await prisma.$queryRawUnsafe<{ passwordHash: string }[]>(
        `SELECT "passwordHash" FROM "User" WHERE email = $1 LIMIT 1`,
        session.email
      );
      if (!users.length) return NextResponse.json({ error: 'User not found.' }, { status: 404 });
    ...
    console.error('[instructor/profile] PATCH:', err);
    return NextResponse.json({ error: 'Server error' }, { status: 500 });
  }
}
