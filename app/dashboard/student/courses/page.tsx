...
function courseProgress(startDate: string, endDate: string) {
  ...
  if (now < start) return { label: 'Upcoming', pct: 0, color: 'bg-blue-500' };
      ...
      <div className="px-6 py-8">
        ...
        {!loading && !error && courses.length === 0 && (
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-white py-20 text-center">
            ...
            <a href="/apply" className="mt-6 rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-700">
              Apply Now
            </a>
          </div>
        )}
        ...
        {!loading && courses.length > 0 && (
          <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {courses.map((course) => {
              ...
              return (
                <div key={course.id} className="group flex flex-col rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:shadow-md hover:border-blue-200">

                  {/* Card header */}
                  <div className="flex items-start justify-between gap-3 border-b border-slate-100 p-5">
                    <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 to-blue-700">
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    <h2 className="mb-1 text-base font-bold text-slate-900 leading-snug group-hover:text-blue-700 transition-colors">
                      {course.name}
                    </h2>
                  ...
                  <div className="border-t border-slate-100 p-4">
                    <a
                      ...
                      className="flex w-full items-center justify-center gap-1.5 rounded-xl bg-blue-50 py-2.5 text-sm font-semibold text-blue-700 transition hover:bg-blue-100"
                    >
                      View Details
